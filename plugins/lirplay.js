...
let handler = async (m, { conn, command, usedPrefix, text, isPrems, isOwner }) => {
  if (!text) return conn.reply(m.chat, `• *Example :* ${usedPrefix + command} alan walker faded`, m)
  let chat = global.db.data.chats[m.chat]
  let results = await yts(text)
  let lirik = await finder("", text)
  let vid = results.all.find(video => video.seconds < 3600)
  if (!vid) return conn.reply(m.chat, 'Video/Audio Not Found', m)
  ...
  for (let i in servers) {
    ...
    try {
      ...
      conn.reply(m.chat, `Server ${server} error!${servers.length >= i + 1 ? '' : '\ntry another server...'}`, m)
    }
  }
  if (yt === false) return conn.reply(m.chat, 'All servers can\'t 😕', m)
...
handler.help = ['lirplay', 'lirplay2'].map(v => v + ' *<teks>*')
