let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!(args[0] || args[1])) return conn.reply(m.chat, `• *Example :* ${usedPrefix + command} 1 2 maka hasilnya adalah surah Al-Fatihah ayat 2`, m)
    if (isNaN(args[0]) || isNaN(args[1])) conn.reply(m.chat, `• *Example :* ${usedPrefix + command} 1 2 maka hasilnya adalah surah Al-Fatihah ayat 2 `, m)
